...
const REVENUE_SCALES = [
  '1億円未満', '1〜3億円', '3〜10億円', '10〜30億円', '30〜100億円', '100億円以上', '分からない／回答したくない',
];

const INDUSTRIES = [
  "ソフトウェア開発", "SaaS・クラウドサービス", "Web制作・デザイン", "システムインテグレーション", "ITコンサルティング", "セキュリティ",
  "食品製造", "繊維・アパレル製造", "化学・医薬品製造", "金属・機械製造", "電子部品・デバイス製造", "自動車・輸送機器製造",
  "百貨店・総合小売", "専門小売（食品）", "専門小売（アパレル）", "専門小売（家電・雑貨）", "EC・オンライン販売", "卸売",
  "飲食店（レストラン）", "カフェ・喫茶店", "居酒屋・バー", "ホテル・旅館", "民泊", "観光・レジャー施設",
  "建設・土木", "建築設計", "不動産売買・仲介", "不動産管理", "リフォーム・リノベーション",
  "病院・クリニック", "歯科医院", "介護・福祉施設", "薬局・ドラッグストア", "整体・鍼灸",
  "学習塾・予備校", "語学教室", "専門学校・各種スクール", "企業研修", "オンライン教育",
  "銀行・信用金庫", "証券", "保険", "ファイナンス",
  "法律事務所", "会計・税理士事務所", "人材紹介・派遣",
  "経営コンサルティング", "マーケティング・PR", "広告代理店", "デザイン事務所",
  "運輸・物流", "美容・エステ", "フィットネス・スポーツ", "清掃・メンテナンス", "イベント・企画", "放送・映像制作", "その他",
];

const CHALLENGE_CATEGORIES = [
  {
    title: 'ブランド戦略',
    options: ['ブランドの方向性が定まっていない', '競合との差別化ができていない', 'ターゲット顧客が明確でない', '価値提案が言語化できていない'],
  },
  {
    title: '認知・集客',
    options: ['新規顧客の獲得が難しい', 'SNS・Web集客がうまくいかない', '口コミ・紹介が少ない', '広告効果が出ない'],
  },
  {
    title: '組織・人材',
    options: ['採用がうまくいかない', '離職率が高い', '社員のモチベーションが低い', 'ブランド理念が社内に浸透していない'],
  },
  {
    title: '売上・収益',
    options: ['売上が伸び悩んでいる', '価格競争に巻き込まれている', 'リピーターが増えない', '単価が上げられない'],
  },
];
      ...
      try {
        const aiResponse = await fetch('/api/analyze-with-ai', {
          ...
          body: JSON.stringify({
            resultId: data.id,
            scores: [
              data.q1_target_insight, data.q2_brand_story, data.q3_brand_personality,
              data.q4_competitive_analysis, data.q5_self_analysis, data.q6_value_proposition,
              data.q7_uniqueness, data.q8_product_uniqueness, data.q9_communication,
              data.q10_inner_branding, data.q11_kpi, data.q12_guideline,
            ],
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8 space-y-8">
          ...
          <section>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">企業名 <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">回答者名 <span className="text-red-500">*</span></label>
                <input type="text" name="respondent_name" value={formData.respondent_name} onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">メールアドレス <span className="text-red-500">*</span></label>
                <input type="email" name="respondent_email" value={formData.respondent_email} onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">事業フェーズ <span className="text-red-500">*</span></label>
                <select name="business_phase" value={formData.business_phase} onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                  <option value="">選択してください</option>
                  {BUSINESS_PHASES.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">売上規模（年間）</label>
                <select name="revenue_scale" value={formData.revenue_scale} onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">選択してください</option>
                  {REVENUE_SCALES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">貴社の企業理念や大切にしている価値観をお聞かせください</label>
              <textarea name="mission" value={formData.mission} onChange={handleInputChange} rows={4}
          ...
          <section>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">3〜5年後、どのような姿を目指していますか？</label>
              <textarea name="vision_future" value={formData.vision_future} onChange={handleInputChange} rows={4}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                placeholder="例：地域で一番愛される工務店になりたい、3年後に売上10億円を突破したい" />
              <p className="mt-1 text-sm text-gray-500">※任意入力</p>
            </div>
          </section>
          ...
          <section>
            ...
            <p className="text-sm text-gray-600 mb-6">現在、特に課題だと感じていることを選択してください（複数選択可）</p>
            <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    {category.options.map(option => (
                      <label key={option} className="flex items-center gap-3 cursor-pointer hover:bg-purple-100 p-2 rounded transition">
                        <input type="checkbox" checked={selectedChallenges.includes(option)}
              ...
              <div className="bg-gray-50 rounded-lg p-5">
                ...
                <label className="flex items-center gap-3 cursor-pointer hover:bg-gray-100 p-2 rounded transition mb-3">
                  <input type="checkbox" checked={showOtherChallenge}
                    onChange={() => handleChallengeToggle('その他')}
                    className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500" />
                  <span className="text-gray-700">その他（自由記述）</span>
                </label>
                {showOtherChallenge && (
                  <textarea name="other_challenge" value={formData.other_challenge} onChange={handleInputChange} rows={3}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
          ...
          <section>
            ...
            <p className="text-sm text-gray-600 mb-6">
              各項目について、現在の状況に最も近いものを5段階で評価してください。<br />
              ...
              {QUESTIONS.map((question, index) => (
                <div key={question.id} className="bg-gray-50 rounded-lg p-6">
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        question.layer === 'ブランド基盤' ? 'bg-purple-100 text-purple-700' :
                        question.layer === '戦略設計' ? 'bg-teal-100 text-teal-700' :
                        'bg-blue-100 text-blue-700'}`}>{question.layer}</span>
                  ...
                  <div className="flex justify-center gap-4">
                    {[1, 2, 3, 4, 5].map(score => (
                      <button key={score} type="button" onClick={() => handleScoreChange(question.id, score)}
          ...
          <section className="bg-blue-50 border-2 border-blue-200 rounded-lg p-6">
            <div className="flex items-start gap-3">
              <input type="checkbox" id="privacy" checked={agreedToPrivacy} onChange={e => setAgreedToPrivacy(e.target.checked)}
                className="w-5 h-5 mt-0.5 text-blue-600" />
              <label htmlFor="privacy" className="text-sm text-gray-700 cursor-pointer">
                <a href="https://huvdesignoffice.com/privacy-policy" target="_blank" rel="noopener noreferrer"
                  className="text-blue-600 hover:underline font-medium">個人情報保護方針</a>
                に同意します。入力いただいた情報は、診断結果の提供およびサービス改善の目的にのみ使用します。
              </label>
            </div>
          </section>
